...
void webserver::dealwithread(int sockfd){
    ...
    if(m_actormodel==1){ 
        ...
        ans.get();
         while (true)
        {
            if (1 == users[sockfd].improv.load())
            {
                if (1== users[sockfd].timer_flag.load())
                {   
                    LOG_ERROR("reactor_model read false");
                    deal_timer(timer, sockfd);
                    users[sockfd].timer_flag.store(0);/*读失败，关闭*/
                }
                users[sockfd].improv.store(0);
                break;
            }
        }
    ...
    void webserver::dealwithwrite(int sockfd){
          ...
          while (true)
        {
            if (1 == users[sockfd].improv.load())
            {
                if (1 == users[sockfd].timer_flag.load())
                {   
                    LOG_ERROR("reactor_model write false");
                    users[sockfd].timer_flag.store(0);/*读失败，关闭*/
                }
                users[sockfd].improv.store(0);
                if(users[sockfd].m_linger==false){
                    deal_timer(timer,sockfd);
                }
                break;
            }
