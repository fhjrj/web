...
void http_conn::initmysql_result(std::shared_ptr<connection_pool> pool){
   ...
   connectionRAII mysqlcon(&mysql,pool);
    if(mysql_query(mysql,"SELECT username,passwd FROM user")){//进行查询，
        LOG_ERROR("SELECT error:%s\n", mysql_error(mysql));
    }

    MYSQL_RES* result=mysql_store_result(mysql);//储存查询结果
    ...
    while(MYSQL_ROW row=mysql_fetch_row(result)){
        ...
        std::string m2(row[1]);
        users[m1]=m2;
    }//将整个表的查询内容（user-password）查询结果储存在users中
} 
    ...
    if(m_Mode==0){
        ...
        if(start==-1){
           if(errno==EAGAIN||errno==EWOULDBLOCK)
             break;//非阻塞跳出循环,然后返回true(没有数据或者数据读完了)
             return false;
        }else if(start==0){
            return false;
        }
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        if (*(p + 1) == '3')
        {
            ...
            if (users.find(name) == users.end())
            {
                ...
                int res = mysql_query(mysql, sql_insert);//没有查询到，将注册结果加入到
        ...
        {
            if (users.find(name) != users.end() && users[name] == password)//密码错误
...
bool http_conn::add_status_line(int status, const char *title)//进行分类
{
